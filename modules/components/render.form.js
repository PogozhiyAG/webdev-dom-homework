...
const initHandlers = () => {
    ...
    const buttonRemoveComment = document.getElementById(
        'button-remove-comment',
    );

    const validationState = {
        isCommentValid: false,
        setIsCommentValid(state) {
            this.isCommentValid = state;
    ...
    const validateAndSend = () => {
        ...
        startLongProcess('Сообщение отправляется...', () =>
            postLoadAndRenderComments(inputComment.value),
        );
        validateComment();
    };
    ...
    const loadAndRenderComments = () => {
        return reloadComments().then(render);
    };
    ...
    const removeLastComment = async () => {
        ...
        await deleteComment(lastComment.id).then(loadAndRenderComments);
    };
    ...
    for (const commentElement of commentElements) {
        commentElement.addEventListener('click', (event) => {
            ...
            inputComment.value = decodeSpecialSymbols(
                `${BEGIN_QUOTE_MARK} ${item.author}: ${item.text} ${END_QUOTE_MARK}`,
            );
            validateComment();
        });
    }
