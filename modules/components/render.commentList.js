...
const initHandlers = () => {
    if (user) {
        ...
        for (const button of likeButtons) {
            button.addEventListener('click', async (event) => {
                ...
                toggleLikeComment(item.id).then(reloadComments).then(render);
            });
        }
    }
    ...
    const editSaveButtons = document.querySelectorAll(
        '.comment .comment-edit-save-button',
    );
    ...
    const editCancelButtons = document.querySelectorAll(
        '.comment .comment-edit-cancel-button',
    );
...
export const renderCommentList = (afterRender) => {
    ...
    const renderCommentText = (comment, index) => {
        ...
        return `<div class="comment-text">${renderCommentBody(
            comment.text,
        )}</div>        
